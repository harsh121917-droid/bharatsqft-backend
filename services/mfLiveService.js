...
function fetchJson(url, headers = {}, timeoutMs = 30000) {
  return new Promise((resolve) => {
    ...
    const req = https.get(url, { headers: reqHeaders, timeout: timeoutMs }, (res) => {
...
async function getLiveHistoricalNav(schemeCode, fallbackScheme = null) {
  ...
  /**
   * Helper to sample points from the chronological list for a given period in days
   */
  const extractPeriodSeries = (daysBack, maxPoints = 30) => {
    let subset = [];
    if (daysBack === null) {
    ...
    const returnPercent = +(((endNav - startNav) / startNav) * 100).toFixed(2);
